...
public:
  ...
  struct WriteAccess {
    WriteAccess(mutex& m, MapType& bucket, const K& key)
      : lock(m)
      , ref_to_value(bucket[key])
    {}
  ...
  struct ReadAccess {
    ReadAccess(mutex& m, const MapType& bucket, const K& key)
      : lock(m)
      , ref_to_value(bucket.at(key))
    {}
  ...
  WriteAccess operator[](const K& key) {
    size_t indx = hasher(key) % buckets.size();
    return {mutexs[indx], buckets[indx], key};
  }
  ...
  bool Has(const K& key) const {
    size_t indx = hasher(key) % buckets.size();
    lock_guard<mutex> lock(mutexs[indx]);
    return buckets[indx].count(key);
  }
