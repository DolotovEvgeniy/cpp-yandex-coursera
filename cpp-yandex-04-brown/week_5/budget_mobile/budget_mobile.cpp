...
public:
  ...
  Budget Collapse(Budget origin, IndexSegment segment) const {
    return {
      origin.earned * tax_.factor + add_.delta * segment.length(),
      origin.spended + spend_.delta * segment.length()
    };//origin - std::max(origin, 0.) * (1. - tax_.factor) + add_.delta * segment.length() - spend_.delta * segment.length();
  }

private:
  // apply tax first, then add and spend
  BulkTaxApplier tax_;
  BulkMoneyAdder add_;
...
class BudgetManager : public SummingSegmentTree<Budget, BulkLinearUpdater> {
public:
...
struct ComputeIncomeRequest : ReadRequest<double> {
  ...
  double Process(const BudgetManager& manager) const override {
    Budget budget = manager.ComputeSum(MakeDateSegment(date_from, date_to));
    return budget.earned - budget.spended;
  }
...
struct PayTaxRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    const auto date_segment = MakeDateSegment(date_from, date_to);
    const double tax_factor = 1.0 - percentage /100.0;
    manager.AddBulkOperation(date_segment, BulkTaxApplier{tax_factor});
  }
