...
void TestSplit() {
  vector<string_view> result = Split("Hello, World!");
...
void TestGetReversedParts() {
  ...
  vector<string> result(range.begin(), range.end());
  vector<string> expected = {"ru", "yandex"};
  ASSERT_EQUAL(result, expected);
}
...
void TestAbsorbSubDomain() {
  ...
  ASSERT(checker.IsSubdomain(Domain("music.ya.ru")));
}

void TestReversedAnswers() {
  vector<Domain> banned_domains = {
    Domain("ya.ru"), Domain("com")
  };
  vector<Domain> domains_to_check = {
    Domain("maps.me"), Domain("google.com"), Domain("m.ya.ru")
  };

  ostringstream out;
  auto answer = CheckDomains(banned_domains, domains_to_check);
