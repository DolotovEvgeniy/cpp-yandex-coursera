...
Node* Next(Node* me) {
  if (isValid(me->right)) {
    Node* next = me->right;
    while (isValid(next->left)) {
      next = next->left;
    }
    return next;
  } if (isValid(me->parent) && me->parent->left == me) {
    return me->parent;
  } if (!isValid(me->right) && isValid(me->parent) && me->parent->right == me) {
    Node* next = me->parent;
    while (isValid(next->parent) && next->parent->left != next) {
      next = next->parent;
    }
    if (isValid(next->parent)) {
      return next->parent;
    } else {
      return nullptr;
    }
