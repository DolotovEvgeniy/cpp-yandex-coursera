...
public:
  ...
  BaseBulkOperation(const SummingSegmentTree<Data, BulkOperation>* tree) : tree_(tree) {}
private:
  const SummingSegmentTree<Data, BulkOperation>* tree_;
};

class BulkLinearUpdater : BaseBulkOperation<MoneyState, BulkLinearUpdater> {
public:
  BulkLinearUpdater(const TreeHolder& tree = nullptr) : BaseBulkOperation(tree.get()) {}

  BulkLinearUpdater(const TreeHolder& tree, const BulkMoneyAdder& add)
      : BaseBulkOperation(tree.get()),
        add_(add)
  {}
